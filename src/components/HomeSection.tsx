    ...
    return (
      <section className="w-full h-svh pt-2 pb-24 md:pb-32 relative overflow-hidden">
        <article className="w-full flex flex-col justify-between h-full">
          ...
          <h1  {...mouseProps} className="text-[20vw] md:text-[15vw] z-1 text-center new-font uppercase unselectable">{t('title')}</h1>
          <main className="grid grid-cols-1 gap-2 md:grid-cols-2 z-1">
            <div className="uppercase font-medium text-xs w-8/12">
              <p>{t('description')}</p>
            </div>
            <div className="z-1">
              <RippleButton className="flex gap-2 items-center border-1 px-20 py-4 text-xs md:text-sm">{t('button')} <ArrowDown className="w-4 h-4" /></RippleButton>
            </div>
          </main>
        </article>
